		...
		<TABLE border=0 width=95%>
			<TR>
				<TD align=left width=30% valign=bottom>[<A
					href="guestlab_list.jsp">자유 게시판(일반모드)</A>]
				</TD>
			...
			<%
							...
							if (what == 2) {
								out.println("<OPTION value=2 selected>휴대폰번호</OPTION>");
								cond = "where phone like '%" + val + "%'";
							} else {
								out.println("<OPTION value=2>휴대폰번호</OPTION>");
							}

							if (what == 3) {
								out.println("<OPTION value=3 selected>주소</OPTION>");
								cond = "where addr like '%" + val + "%'";
							} else {
								out.println("<OPTION value=3>주소</OPTION>");
							}

							if (what == 4) {
								out.println("<OPTION value=4 selected>이름+휴대폰번호</OPTION>");
								cond = "where ename like '%" + val + "%' or addr like '%" + val + "%'";
							} else {
								out.println("<OPTION value=4>이름+휴대폰번호</OPTION>");
							}

							if (what == 5) {
								out.println("<OPTION value=5 selected>이름+주소</OPTION>");
								cond = "where ename like '%" + val + "%' or addr like '%" + val + "%'";
							} else {
								out.println("<OPTION value=5>이름+주소</OPTION>");
							}

							if (what == 6) {
								out.println("<OPTION value=6 selected>휴대폰번호+주소</OPTION>");
								cond = "where phone like '%" + val + "%' or addr like '%" + val + "%'";
							} else {
								out.println("<OPTION value=6>휴대폰번호+주소</OPTION>");
							}

							if (what == 7) {
								out.println("<OPTION value=7 selected>이름+휴대폰번호+주소</OPTION>");
								cond = "where ename like '%" + val + "%' or phone like '%" + val + "%' ";
								cond += "or addr like '%" + val + "%'";
							} else {
								out.println("<OPTION value=7>이름+휴대폰번호+주소</OPTION>");
							}
							%>
	...
	<CENTER>
		...
		<TABLE border=0 width=600 cellpadding=4 cellspacing=0>
			...
			<tr align="center" bgcolor="#87E8FF">
			   <td width="62" bgcolor="#DFEDFF"><font size="2">번호</font></td>
			   <td width="140" bgcolor="#DFEDFF"><font size="2">이름</font></td>
			   <td width="184" bgcolor="#DFEDFF"><font size="2">폰</font></td>
			   <td width="78" bgcolor="#DFEDFF"><font size="2">성별</font></td>
			   <td width="149" bgcolor="#DFEDFF"><font size="2">주소</font></td>
			  </tr>
			<tr align="center">
			...
			totalrows = ename.size(); //DB에서 가져온 총 레코드의 개수 (tatalrows = 24)		
			...
			<tr align="center" bgcolor="#87E8FF">
				<td width="42" bgcolor="#DFEDFF"><font size="2"><%=idx.elementAt(j)%></font></td>
				<td width="340" bgcolor="#DFEDFF"><font size="2"><%=ename.elementAt(j)%></font></td>
				<td width="84" bgcolor="#DFEDFF"><font size="2"><%=phone.elementAt(j)%></font></td>
				<td width="78" bgcolor="#DFEDFF"><font size="2"><%=gender.elementAt(j)%></font></td>
				<td width="49" bgcolor="#DFEDFF"><font size="2"><%=addr.elementAt(j)%></font></td>
			</tr>
			...
			<tr>
				<td colspan="5" align="center">
					...
					if (wheregroup > 1) { //현재 나의 페이지 그룹이 1이상일 때 처음
						out.println("[<a href='guestlab_search.jsp?gogroup=1");
						out.println("&stype=" + what + "&sval=" + val + "'>처음</a>]");
						out.println("[<a href='guestlab_search.jsp?gogroup=" + priorgroup);
					...
					if (ename.size() != 0) { //name.size() : 총 레코드의 개수가 0이 아니라면
						for (int jj = startpage; jj <= endpage; jj++) {
							if (jj == where) { //jj가 자신의 페이지 번호라면 링크없이 출력
						out.println("[" + jj + "]");
							} else { //jj가 현재 자신의 페이지 번호가 아니라면 링크를 걸어서 출력
						out.println("[<a href='guestlab_search.jsp?go=" + jj);
					...
					if (wheregroup < totalgroup) { //링크 처리
						out.println("[<a 'href=guestlab_search.jsp?gogroup=" + nextgroup);
						out.println("&stype=" + what + "&sval=" + val + "'>다음</a>]");
						out.println("[<a 'href=guestlab_search.jsp?gogroup=" + totalgroup + ">마지막</a>]");
